...
<template>
    <v-card density="compact">
        <v-card-title class="d-flex ga-2">
            ...
            <v-btn density="compact" variant="tonal" color="" @click="zero" prependIcon="mdi-close-circle-multiple">Zero
                ALL</v-btn>
            ...
            <tbody>
                <tr>
                    <td class="py-2 border-e" v-for="(weight, index) in model.weights" :key="index">
                        W<sub>{{ index + 1 }}</sub><br /><code>{{ weight }}</code>
                        <sup>
